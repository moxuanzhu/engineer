    ...
    <div class="dijitDialog" id="addMapModalDialog" v-bind:style="{top: modal_top+'px', left: modal_left+'px'}" style="width: 620px; height: 430px; position: fixed; opacity: 1; z-index: 2000;background-color: #fff">
      ...
      <div class="dijitDialogPaneContent formCss" style="padding: 0px; width:620px; height:334px;">
        <div style="width:620px;height:333px; overflow-y:scroll;margin-top: 10px;">
          <el-table :data="constructionList"  height="250" border style="width: 100%">
            <el-table-column prop="fdFanNumber" label="风机编号" >
            </el-table-column>
            <el-table-column prop="fdSubordinateLine" label="所属线路" >
            </el-table-column>
            <el-table-column prop="fdThroughTime" label="贯通时间">
            </el-table-column>
            <el-table-column prop="fdExcavation" label="具备开挖条件">
            </el-table-column>
          </el-table>
        </div>
      </div>
...
import '@/assets/js/city-data.js'
  ...
  export default{
    data(){
      return {
        title:this.$parent.constructionModelAttr.title,//弹出框表头
    ...
    methods:{
      ...
      getConstructionList(){   //查询后回显详情
        ...
        this.$http.post('/engineer/selectConstructionRecord', params)
          .then(res => {
            _this.constructionList=res.data.data;
      ...
      close() {  //重置
        this.$parent.constructionModelAttr.showModel = false;    //关闭弹出框
      },
    },
    components:{
