...
<div style="width: 99%;height: 100%;border: 1px solid #EAF0F4;">
  <div style="height: 50px;background-color:#F9FAFB;position: relative; ">
    <span style="position: absolute;font-size: larger;font-weight: bold;margin-top: 12px;margin-left: 13px;"> 风机安装工程日历及工期甘特图分析表</span>
  </div>
    ...
    export default {
      components: {
        ...
        methods: {
          ...
          getGanntDataList(){
            var _this=this;
            this.$http.post('/engineer/getFanCalendarAnalysisGanttChart', {fdStageName:this.$parent.$parent.$parent.activeName,fdProid:this.$parent.$parent.$parent.popData.fdProid})
              .then(res => {
                console.log(res);
