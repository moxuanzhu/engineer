      ...
      components: {
        ...
        methods: {
            ...
            if(data.type=="work"){
              for(var i=0;i<box.othersClum.length;i++){
                ...
                for(var j=0;j<box.othersClum[i].chrildren.length;j++){
                  ...
                  if(new Date(iDate).getTime()>=new Date(data.startDate).getTime() && new Date(iDate).getTime()<=new Date(data.endDate).getTime()){
            ...
            for(var i=0;i<data.length;i++){
              if(data[i].type=="plan"){
                for(var j=0;j<data[i].times.length;j++){
                  data[i].times[j].startDate=new Date(data[i].times[j].fdPlanStartTime ).getFullYear()+"-"+(new Date(data[i].times[j].fdPlanStartTime ).getMonth() + 1>9?new Date(data[i].times[j].fdPlanStartTime ).getMonth() + 1:"0"+(new Date(data[i].times[j].fdPlanStartTime ).getMonth() + 1))+"-"+(new Date(data[i].times[j].fdPlanStartTime ).getDate()>9?new Date(data[i].times[j].fdPlanStartTime ).getDate():"0"+new Date(data[i].times[j].fdPlanStartTime ).getDate());
                  data[i].times[j].endDate=new Date(data[i].times[j].fdPlanEndTime ).getFullYear()+"-"+(new Date(data[i].times[j].fdPlanEndTime ).getMonth() + 1>9?new Date(data[i].times[j].fdPlanEndTime ).getMonth() + 1:"0"+(new Date(data[i].times[j].fdPlanEndTime ).getMonth() + 1))+"-"+(new Date(data[i].times[j].fdPlanEndTime ).getDate()>9?new Date(data[i].times[j].fdPlanEndTime ).getDate():"0"+new Date(data[i].times[j].fdPlanEndTime ).getDate());
                  if(!json.startDate || new Date(json.startDate).getTime() > new Date(data[i].times[j].startDate).getTime()){
                    json.startDate=data[i].times[j].startDate;
                  }
                  if(!json.endDate || new Date(json.endDate).getTime() < new Date(data[i].times[j].endDate).getTime()){
          ...
          putShowText(data){
            for(var i=0;i<data.length;i++){
              if(data[i].type=="work"){
                  var startDate=new Date(data[i].startDate).getFullYear()+"-"+(new Date(data[i].startDate).getMonth() + 1>9?new Date(data[i].startDate).getMonth() + 1:"0"+(new Date(data[i].startDate).getMonth() + 1))+"-"+(new Date(data[i].startDate).getDate()>9?new Date(data[i].startDate).getDate():"0"+new Date(data[i].startDate).getDate());
                  var endDate=new Date(data[i].endDate).getFullYear()+"-"+(new Date(data[i].endDate).getMonth() + 1>9?new Date(data[i].endDate).getMonth() + 1:"0"+(new Date(data[i].endDate).getMonth() + 1))+"-"+(new Date(data[i].endDate).getDate()>9?new Date(data[i].endDate).getDate():"0"+new Date(data[i].endDate).getDate());
                  //进行少天
          ...
          getGanntDataList(){
            var _this=this;
            this.$http.post('/engineer/getFanCalendarGanttChart', {fdStageName:this.$parent.$parent.$parent.activeName,fdProid:this.$parent.$parent.$parent.popData.fdProid})
              .then(res => {
                console.log(res);
            ...
            //获取后台给出的数据中最早的开始时间和最晚的结束时间
            /*var planList=[
              {name:"计划工期一",status:false,type:"plan",times:[{startDate:"2018-07-16",endDate:"2018-09-20"}]},
              {name:"计划工期二",status:false,type:"plan",times:[{startDate:"2018-07-19",endDate:"2018-09-10"}]},
              {name:"计划工期三",status:true,type:"plan",
                times:[
                {startDate:"2018-07-20",endDate:"2018-07-25"},
                {startDate:"2018-08-01",endDate:"2018-08-19"}
                ]
              }
            ];
            var dataList=[
              { number:"1#",line:"A线",type:"work",
                works:[
            ...
            //根据后台数据计算每个表格的提示信息
            var data=_this.putShowText(dataList);
            planList.push({number:"风机编号",line:"所属线路",type:"work"});
            for(var i=0;i<data.length;i++){
              planList.push(data[i]);
            }
            //将后台数据根据日期填入上面生成的二维表数据结构中
