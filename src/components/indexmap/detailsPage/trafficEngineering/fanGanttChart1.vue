      ...
      components: {
        ...
        methods: {
                ...
                for(var k=0;k<box.othersClum[j].chrildren.length;k++){
                  ...
                  for(var z=0;z<data.works.length;z++){
                    ...
                    for(var i=0;i<data.works[z].dataList.length;i++){
                      ...
                      var dayDate="";
                      if(dayD.dataList[i].fdPitExcavation){
                        dayDate=dayD.dataList[i].fdPitExcavation;
                      }else if(dayD.dataList[i].fdFoundationTreatment){
                        dayDate=dayD.dataList[i].fdFoundationTreatment;
                      }else if(dayD.dataList[i].fdFoundationPouring){
                        dayDate=dayD.dataList[i].fdFoundationPouring;
                      }else if(dayD.dataList[i].fdTemplatesInstalling){
                        dayDate=dayD.dataList[i].fdTemplatesInstalling;
                      }else if(dayD.dataList[i].fdBarBinding){
                        dayDate=dayD.dataList[i].fdBarBinding;
                      }else if(dayD.dataList[i].fdCushionPouring){
                        dayDate=dayD.dataList[i].fdCushionPouring;
                      }

                      if(dayDate==iDate){
                        box.othersClum[j].chrildren[k].value=dayD.dataList[i].fdFanNumber;
          ...
          getMinDate(data){
            var date="";
            for(var i=0;i<data.length;i++){
                if(data[i].fdPitExcavation){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdPitExcavation).getTime()){
                    date=data[i].fdPitExcavation;
                  }
                }else if(data[i].fdFoundationTreatment){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdFoundationTreatment).getTime()){
                    date=data[i].fdFoundationTreatment;
                  }
                }else if(data[i].fdFoundationPouring){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdFoundationPouring).getTime()){
                    date=data[i].fdFoundationPouring;
                  }
                }else if(data[i].fdTemplatesInstalling){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdTemplatesInstalling).getTime()){
                    date=data[i].fdTemplatesInstalling;
                  }
                }else if(data[i].fdBarBinding){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdBarBinding).getTime()){
                    date=data[i].fdBarBinding;
                  }
                }else if(data[i].fdCushionPouring){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdCushionPouring).getTime()){
                    date=data[i].fdCushionPouring;
                  }
                }
            }
            return date;
          ...
          getGanntDataList(){
            var _this=this;
            this.$http.post('/engineer/getDraughtFanGanttChart', {fdStageName:this.$parent.$parent.$parent.activeName,fdProid:this.$parent.$parent.$parent.popData.fdProid})
              .then(res => {
                console.log(res);
