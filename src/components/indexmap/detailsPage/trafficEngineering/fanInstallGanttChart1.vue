      ...
      components: {
        ...
        methods: {
                ...
                for(var k=0;k<box.othersClum[j].chrildren.length;k++){
                  ...
                  for(var z=0;z<data.works.length;z++){
                    ...
                    for(var i=0;i<data.works[z].dataList.length;i++){
                      ...
                      var dayDate="";
                      if(dayD.dataList[i].fdVaneHoistingTime){
                        dayDate=dayD.dataList[i].fdVaneHoistingTime;
                      }else if(dayD.dataList[i].fdFoundationPourings){
                        dayDate=dayD.dataList[i].fdFoundationPourings;
                      }else if(dayD.dataList[i].fdBoxVariableInstallation){
                        dayDate=dayD.dataList[i].fdBoxVariableInstallation;
                      }
          ...
          getMinDate(data){
            var date="";
            for(var i=0;i<data.length;i++){
                if(data[i].fdVaneHoistingTime){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdVaneHoistingTime).getTime()){
                    date=data[i].fdVaneHoistingTime;
                  }
                }else if(data[i].fdFoundationPourings){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdFoundationPourings).getTime()){
                    date=data[i].fdFoundationPourings;
                  }
                }else if(data[i].fdBoxVariableInstallation){
                  if(!date || new Date(date).getTime() > new Date(data[i].fdBoxVariableInstallation).getTime()){
                    date=data[i].fdBoxVariableInstallation;
                  }
                }
            }
            return date;
          ...
          getGanntDataList(){
            var _this=this;
            this.$http.post('/engineer/getFanInstallationGanttChart', {fdStageName:this.$parent.$parent.$parent.activeName,fdProid:this.$parent.$parent.$parent.popData.fdProid})
              .then(res => {
                console.log(res);
