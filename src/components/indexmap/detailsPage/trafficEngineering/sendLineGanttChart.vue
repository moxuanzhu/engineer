        ...
        methods: {
          ...
          chengenum1(item,item1,item2){
            ...
            if(document.getElementById(inputid)){
              ...
              if(this.ganttData[item.$index].valType=="1" || this.ganttData[item.$index].valType=="2"){
              ...
              var inputValue = inputValue.replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3')
               var date=this.ganttData[item.$index].othersClum[item1].firstLable+ "-"+(parseInt(this.ganttData[item.$index].othersClum[item1].chrildren[item2].clumlable)>9? this.ganttData[item.$index].othersClum[item1].chrildren[item2].clumlable:"0"+ this.ganttData[item.$index].othersClum[item1].chrildren[item2].clumlable);
                ...
                if(this.ganttData[item.$index].valType=="1"){
                  param.fdFoundationPouringId=this.ganttData[item.$index].othersClum[item1].chrildren[item2].id;
                  param.fdFoundationPouring=inputValue;
                }else if(this.ganttData[item.$index].valType=="2"){
                  param.fdLineTowerId=this.ganttData[item.$index].othersClum[item1].chrildren[item2].id;
                  param.fdLineTower=inputValue;
                }else if(this.ganttData[item.$index].valType=="3"){
                  param.fdTightenLineId=this.ganttData[item.$index].othersClum[item1].chrildren[item2].id;
                  param.fdTightenLine=inputValue;
                }
                this.$http.post('/engineer/insertGiveGanttChartInput', param)
                  .then(res => {
                ...
                for(var k=0;k<box.othersClum[j].chrildren.length;k++){
                  ...
                  for(var z=0;z<data.works.length;z++){
                    ...
                    for(var i=0;i<data.works[z].dataList.length;i++){
                      ...
                      var dayDate=""
                      if(dayD.dataList[i].fdLineTowerDate){
                        dayDate=dayD.dataList[i].fdLineTowerDate;
                        if(dayDate==iDate){
                          box.othersClum[j].chrildren[k].value=dayD.dataList[i].fdLineTower;
                          box.othersClum[j].chrildren[k].id=dayD.dataList[i].fdLineTowerId;
                        ...
                        if(dayDate==iDate){
                          box.othersClum[j].chrildren[k].value=dayD.dataList[i].fdFoundationPouring;
                          box.othersClum[j].chrildren[k].id=dayD.dataList[i].fdFoundationPouringId;
                          box.othersClum[j].chrildren[k].fdScheduleid=dayD.fdScheduleid;
                        }
          ...
          getMinDate(data){
            var date="";
            for(var i=0;i<data.length;i++){
              if(data[i].fdLineTowerDate){
                if(!date || new Date(date).getTime() > new Date(data[i].fdLineTowerDate).getTime()){
                  date=data[i].fdLineTowerDate;
                }
          ...
          getMaxDate(data){
            var date="";
            for(var i=0;i<data.length;i++){
              if(data[i].fdFoundationPouringDate){
                if(!date || new Date(date).getTime() < new Date(data[i].fdFoundationPouringDate).getTime()){
                  date=data[i].fdFoundationPouringDate;
                }
              }else if(data[i].fdLineTowerDate){
                if(!date || new Date(date).getTime() < new Date(data[i].fdLineTowerDate).getTime()){
                  date=data[i].fdLineTowerDate;
                }
              }else if(data[i].fdTightenLineDate){
          ...
          getGanntDataList(){
            var _this=this;
            this.$http.post('/engineer/getSendOutGanttChart', {fdStageName:this.$parent.$parent.$parent.activeName,fdProid:this.$parent.$parent.$parent.popData.fdProid})
              .then(res => {
                console.log(res);
